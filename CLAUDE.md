...
kovacevicapp/
  ...
  docker/            # (noch nicht angelegt — Deployment läuft aktuell direkt via Coolify
                     #  gegen backend/'s eigenes Dockerfile, kein docker-compose.yml im Repo,
                     #  siehe "Infrastruktur (NAS)")
```
...
4. **Dokumente** — Datei-Upload (PDF/Office/Bilder, max. 20 MB) mit Zuweisung an ein Familienmitglied, Download, Umbenennen, Löschen. Für PDF/Bild-Dokumente zusätzlich: **Termine aus Dokument extrahieren** — Claude API analysiert das Dokument, Nutzer prüft/bearbeitet die Vorschläge in einem Review-Dialog, erst nach Bestätigung werden Kalendertermine angelegt (kein Autocreate ohne Review)
5. **Benachrichtigungen (Android)** — täglich um 21:00 Uhr eine Push-Benachrichtigung mit den morgigen Kalenderterminen (kein Versand, wenn morgen keine Termine anstehen); Zustellung über einen selbstgehosteten ntfy-Server (kein Firebase/FCM), Tap auf die Benachrichtigung öffnet die App direkt im Kalender

---

## Key Patterns

- **Router-First:** DB-Zugriffe laufen direkt im Router (`AsyncSession` inline) — kein DB-Repository-Layer. `app/services/` (seit "Termine aus Dokument extrahieren") kapselt stattdessen Business-Logik ohne direkten DB-Bezug, aktuell die Claude-API-Anbindung (`document_extraction.py`) und den täglichen Benachrichtigungs-Versand (`notifications.py`); Router bleiben dünn und rufen den Service auf
- **Scheduler (seit "Benachrichtigungen"):** `app/main.py`s `lifespan()` startet einen in-process `AsyncIOScheduler` (APScheduler) für wiederkehrende, zeitgebundene Jobs — erster echter Cron-artiger Trigger der Codebasis (bisherige "Auto-Cleanup"-Logik lief bewusst lazy beim nächsten GET, kein Scheduler nötig, da kein exakter Zeitpunkt gefordert war). Wichtig für Tests: `conftest.py`s `client`-Fixture nutzt `ASGITransport`, das **kein** `lifespan` auslöst — Scheduler-Jobs sind darüber nicht testbar, Services müssen daher eigenständig (mit injizierbarer `session_factory`) unit-testbar sein, unabhängig vom Scheduler-Wiring
...
| `backend/app/services/notifications.py` | Täglicher 21:00-Uhr-Job (via APScheduler aus `main.py`): ermittelt morgige Kalendertermine und published sie an einen selbstgehosteten ntfy-Topic (`NTFY_URL`/`NTFY_TOPIC`) — no-op/Log wenn nicht konfiguriert oder keine Termine morgen; kein `device_tokens`-Modell (ntfy-Pub/Sub statt FCM-Gerätetracking) |
...
| `NTFY_URL` | Basis-URL des selbstgehosteten ntfy-Servers für die tägliche 21:00-Uhr-Benachrichtigung — ohne Wert bleibt der Versand ein stiller No-op (kein Fehler) | No |
| `NTFY_TOPIC` | ntfy-Topic-Name für den Benachrichtigungs-Versand (Default `familio-events`) | No |
...
- **Tatsächliches Deployment:** Coolify baut/deployt `backend/`'s eigenes Dockerfile direkt — es gibt **kein** `docker/docker-compose.yml` im Repo (der ursprüngliche Plan aus der Monorepo-Struktur ist bisher nicht umgesetzt; PostgreSQL/Nginx/Reverse-Proxy laufen als separate, außerhalb dieses Repos verwaltete Coolify-Services). Agents sollten nicht annehmen, dass eine Compose-Datei existiert oder erweitert werden kann
- HTTPS über Nginx + Let's Encrypt (oder lokales Zertifikat)
- Android App verbindet sich über lokale IP oder DynDNS
- **ntfy (seit "Benachrichtigungen"):** selbstgehosteter ntfy-Server für Push-Benachrichtigungen ist noch **nicht deployt** — muss manuell (z. B. eigener Coolify-Service mit `binwiederhier/ntfy`-Image) aufgesetzt werden, danach `NTFY_URL`/`NTFY_TOPIC` am Backend setzen (siehe Backlog FS-36)
