...
- [ ] FS-16: Render-Tests für MonthView, WeekView, EventFormModal, CalendarPage — **teilweise erledigt:** `EventFormModal` (Edit-Vorausfüllen + Ganztägig-Verhalten) und `WeekView` (Ganztägig-Zeile) haben jetzt Render-Tests via "Ganztägige Termine"; `MonthView` und `CalendarPage` weiterhin ungetestet
...
- [ ] FS-33: Android — "Ganztägig"-Option im Kalender nachziehen (Web-only in v1) (introduced in: Ganztägige Termine)
