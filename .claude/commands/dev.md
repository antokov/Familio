...
4. `.claude/workspace/design-decision.md` — **UI tasks only:** concrete design decisions you MUST follow (if file exists)

Also read the reference files listed in `arch-decision.md` to understand existing patterns.

**UI rule:** If `design-decision.md` exists, every CSS/layout/token choice in your implementation must be traceable to a decision in that file. Deviations must be logged in `impl-report.md` under "Deviations".
